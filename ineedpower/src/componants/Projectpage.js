...
class ProjectData extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(
      `http://localhost:5000/participantrequest/${this.props.project.projectId}`
    )
      .then(res => res.json())
  ...
  handleClick() {
    ...
    for (
      let index = 0;
      index < this.state.participantrequests.length;
      index++
    ) {
      ...
      return (
        <div>
          <div className="paragraafEditProj">
            <div className="stylish" onClick={this.handleClick}>
              <Popup />
            </div>
      ...
      return (
        <div>
          <div className="rightButton">
            <NavLink to="/JoinProject" className="buttonEditProj">
              JOIN
            </NavLink>
          </div>
          <div className="paragraafEditProj">
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="profileContainer">{competenceList}</div>
      </div>
    );
  }
}
...
class ProjectLike extends React.Component {
  ...
  componentDidMount() {
    fetch(`http://localhost:5000/projectlikes/${this.props.id}`)
      .then(res => res.json())
      .then(res =>
        this.setState({ likes: res }, function() {
  ...
  onClick() {
    if (!this.state.liked) {
      ...
      fetch(
        `http://localhost:5000/projectlike/add/${this.props.id}/${
          this.props.user
        }`
      );
    } else {
      ...
      fetch(
        `http://localhost:5000/projectlike/delete/${this.props.id}/${
          this.props.user
        }`
      );
    }
  ...
  render() {
    return (
      <div>
        <i className={this.state.class} onClick={this.onClick} />
  ...
  render() {
    ...
    if (this.state.fetched) {
      linksList = this.state.links.map(link => (
        <div className="profileLink" key={link.projectLinkId}>
    ...
    return (
      <div>
        ...
        <div className="profileContainer">{linksList}</div>
      </div>
    );
  }
}
    ...
    if (this.state.fetched) {
      ...
      return (
        <div>
          ...
          <div className="profileContainer">{ProblemList}</div>
        </div>
      );
    }
    return <p />;
...
class Projectpage extends React.Component {
  ...
  componentDidMount() {
    fetch(
      `http://localhost:5000/displayProject/${this.props.location.hash.substr(
        1
      )}`
    )
      .then(res => res.json())
    ...
    if (this.state.fetched) {
      ...
      return (
        <div>
          ...
          <Users
            fetch={`http://localhost:5000/project/participants/${id}`}
            title="Participants"
          />
          <ProjectLinks id={id} />
          <Tags id={id} />
